import { useState } from 'react';
import { Bubble, Snapshot } from '../../types';
import { fmtDate } from '../../utils/helpers';

interface SnapshotModalProps {
  isOpen: boolean;
  onClose: () => void;
  bubbles: Bubble[];
  snapshots: Snapshot[];
  setSnapshots: (snapshots: Snapshot[]) => void;
  onViewChange: (view: 'progress') => void;
}

export default function SnapshotModal({
  isOpen,
  onClose,
  bubbles,
  snapshots,
  setSnapshots,
  onViewChange,
}: SnapshotModalProps) {
  const [name, setName] = useState('');
  const [note, setNote] = useState('');

  if (!isOpen) return null;

  const handleSave = () => {
    if (bubbles.length === 0) {
      alert('Your map is empty — add some bubbles first.');
      return;
    }

    const snapshotName = name.trim() || fmtDate(new Date());
    const newSnapshot: Snapshot = {
      id: Date.now(),
      name: snapshotName,
      note: note.trim(),
      date: new Date().toISOString(),
      bubbles: JSON.parse(JSON.stringify(bubbles)),
      source: 'manual',
    };

    setSnapshots([newSnapshot, ...snapshots]);
    setName('');
    setNote('');
    onClose();
    onViewChange('progress');
  };

  const handleBackdropClick = (e: React.MouseEvent) => {
    if (e.target === e.currentTarget) {
      onClose();
    }
  };

  return (
    <div
      onClick={handleBackdropClick}
      className="fixed inset-0 flex items-center justify-center p-4 z-[1000] animate-[fadeIn_0.14s_ease]"
      style={{ background: 'rgba(28,26,22,0.4)' }}
    >
      <div
        className="rounded-[13px] p-6 w-full max-w-[390px] border animate-[slideUp_0.17s_cubic-bezier(0.25,0,0,1)]"
        style={{ background: 'var(--bg)', borderColor: 'var(--rule)', boxShadow: 'var(--sh-lg)' }}
      >
        <div style={{ fontFamily: 'var(--font-h)', color: 'var(--ink)' }} className="text-[1.05rem] italic mb-1">
          Take a Snapshot
        </div>
        <div className="text-[0.7rem] leading-relaxed mb-4" style={{ color: 'var(--ink3)' }}>
          Freeze your map right now. Come back later to see what changed.
        </div>

        <div className="mb-3">
          <label className="text-[0.65rem] font-medium tracking-[0.08em] uppercase block mb-1" style={{ color: 'var(--ink3)' }}>
            Name
          </label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. March 2025, After the move…"
            maxLength={50}
            className="w-full text-[0.78rem] rounded-lg px-3 py-1.5 border outline-none transition-all duration-150"
            style={{
              fontFamily: 'var(--font-b)',
              background: 'var(--bg2)',
              borderColor: 'var(--rule)',
              color: 'var(--ink)',
            }}
          />
        </div>

        <div className="mb-4">
          <label className="text-[0.65rem] font-medium tracking-[0.08em] uppercase block mb-1" style={{ color: 'var(--ink3)' }}>
            Note to your future self <span className="font-light normal-case tracking-normal">(optional)</span>
          </label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="What are you feeling right now? What do you want to change?"
            className="w-full text-[0.78rem] rounded-lg px-3 py-1.5 border outline-none transition-all duration-150 resize-vertical min-h-[62px] leading-relaxed"
            style={{
              fontFamily: 'var(--font-b)',
              background: 'var(--bg2)',
              borderColor: 'var(--rule)',
              color: 'var(--ink)',
            }}
          />
        </div>

        <div className="flex gap-2 justify-end">
          <button
            onClick={onClose}
            className="text-[0.71rem] font-medium px-3 py-1.5 rounded-md border transition-all duration-150 tracking-wide hover:opacity-80"
            style={{
              fontFamily: 'var(--font-b)',
              borderColor: 'var(--rule)',
              background: 'var(--bg)',
              color: 'var(--ink2)',
            }}
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="text-[0.71rem] font-medium px-3 py-1.5 rounded-md border-none transition-all duration-150 tracking-wide hover:opacity-90"
            style={{
              fontFamily: 'var(--font-b)',
              background: 'var(--gold)',
              color: '#fff',
            }}
          >
            Save snapshot
          </button>
        </div>
      </div>
    </div>
  );
}
